...
int main(void) {
    ...
    int current = 0;
    int secured_prize = 0; // palier sécurisé (ex: après Q5 et Q10)
    int used_5050 = 0, used_phone = 0, used_audience = 0;

    printf("Bienvenue dans 'Le million' !\n");
    printf("Répondez aux questions pour gagner de l'argent. Vous avez 3 jokers : 50/50, Appel à un ami, Sondage du public.\n");
    ...
    while (current < total) {
        ...
        while (!answered) {
            ...
            if (choice == -1) {
                // quitter
                printf("Vous quittez avec %d €.\n", (current == 0) ? 0 : prizes[current-1]);
                return 0;
            } else if (choice == -2) {
                // jokers menu
                printf("\nJokers disponibles :\n");
                printf("1) 50/50 %s\n", used_5050 ? "(utilisé)" : "");
                printf("2) Appel à un ami %s\n", used_phone ? "(utilisé)" : "");
                printf("3) Sondage du public %s\n", used_audience ? "(utilisé)" : "");
                printf("Choisissez 1/2/3 ou autre pour annuler : ");
                char line[32];
                if (!fgets(line, sizeof(line), stdin)) { printf("Erreur entrée.\n"); continue; }
                int sel = line[0] - '0';
                if (sel == 1 && !used_5050) {
                    int out[2];
                    if (joker_5050(current, out)) {
                        ...
                        used_5050 = 1;
                        printf("50/50 appliqué. Deux choix retirés.\n");
                    ...
                    printf("Joker invalide ou déjà utilisé.\n");
                }
            } else if (choice >= 0 && choice < 4) {
                // vérifier réponse
                if (choice == q->correct) {
                    printf("Bonne réponse ! Vous gagnez %d €.\n", prizes[current]);
                    // palier sécurisé
                    if (current == 4) secured_prize = prizes[4]; // après Q5
                    if (current == 9) secured_prize = prizes[9]; // après Q10
                    current++;
                    answered = 1;
                    if (current == total) {
                        printf("Félicitations ! Vous avez gagné 1 000 000 € !\n");
                        return 0;
                    }
                } else {
                    printf("Mauvaise réponse. La bonne réponse était %c) %s\n", 'A' + q->correct, q->choices[q->correct]);
                    printf("Vous repartez avec %d € (palier sécurisé).\n", secured_prize);
                    return 0;
                }
            } else {
